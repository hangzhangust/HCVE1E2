function [r1,r2,r3,p1,p2,p3] = verify_param_im(Jstore_mat,msa_bin_unique,weight_seq_unique,num_mutants_combine_array)
...
[r2,p2] = corr(double_MSA_flat,double_MCMC_flat,'type','Pearson');

plot(arrayline,arrayline,'k')
xlabel('Double mutant probability (MSA)')
ylabel({'Double mutant', 'probability (IM)'})
...
[r3,p3] = corr(connected_MSA_flat,connected_MCMC_flat,'type','Pearson');
plot(arrayline,arrayline,'k')
xlabel('Connected correlation (MSA)')
ylabel({'Connected correlation','(IM)'})
...
% h = legend('MSA','Model','Location ','best');
...
ylim([1e-4 1])
[rho,p] = corr(eps_MSA',eps_MCMC','type','Pearson')
epsilon_max
FIG.Name = 'num_3a';
grid off
box off
